...
- Windows ログオン時の自動起動をインストーラー管理へ変更
- アンインストール時に自動起動ショートカットも削除されるよう修正
- 旧版や開発時に残った `Run\RemoteCamera` レジストリ値を、新版インストール時に削除
- 開発用の実行ファイルがスタートアップへ自動登録されないよう修正

インストーラーはコード署名していないため、SmartScreen の警告が表示される場合があります。詳細は「証明書なしのインストーラー警告について」を確認してください。
