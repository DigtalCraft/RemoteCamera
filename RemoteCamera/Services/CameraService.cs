...
namespace RemoteCamera
{
    ...
    internal sealed class CameraService : IAsyncDisposable
    {
        ...
        public async Task InitializeAsync(CameraDeviceOption? deviceOption = null)
        {
            ...
            try
            {
                await ActivateOpenedCameraAsync(openedCapture, selectedDevice);
            }
            catch
            {
        ...
        private async Task ActivateOpenedCameraAsync(VideoCapture openedCapture, CameraDeviceOption selectedDevice)
        {
            ...
            lock (syncRoot)
            {
                ...
                selectedCaptureIndex = selectedDevice.CaptureIndex;
                previewEnabled = true;
        ...
        private string BuildStatusText()
        {
            ...
            var deviceName = selectedDeviceName ?? "USBカメラ";
            var baseText = isRecording && !string.IsNullOrWhiteSpace(recordingPath)
                ? $"録画中: {Path.GetFileName(recordingPath)}"
                : $"{deviceName} を使用中";
