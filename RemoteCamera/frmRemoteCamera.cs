...
namespace RemoteCamera
{
    ...
    public partial class frmRemoteCamera : Form
    {
        ...
        private bool networkCameraSelectionSyncing;
        ...
        public frmRemoteCamera()
        {
            ...
            buttonApplyNetworkCamera.Click += ApplyNetworkCameraButton_Click;
            buttonRefreshDevices.Click += RefreshDevicesButton_Click;
            buttonNetworkCameraMaster.Click += NetworkCameraMasterButton_Click;
            ...
            networkCameraComboBox.SelectedIndexChanged += NetworkCameraComboBox_SelectedIndexChanged;
        ...
        private async Task InitializeStartupAsync()
        {
            ...
            try
            {
                ...
                {
                    WriteStartupLog($"InitializeStartupAsync: network camera start: {selectedNetworkCamera.DisplayName}");
                    await cameraService.InitializeAsync(selectedNetworkCamera);
                }
        ...
        /// 選択中のネットワークカメラへ切り替える。
        /// </summary>
        private async void ApplyNetworkCameraButton_Click(object? sender, EventArgs e)
        {
            await SwitchSelectedNetworkCameraAsync();
        }
        ...
        /// <summary>
        /// ネットワークカメラ変更時にカメラを即時切り替える。
        /// </summary>
        private async void NetworkCameraComboBox_SelectedIndexChanged(object? sender, EventArgs e)
        {
            if (networkCameraSelectionSyncing)
            {
                return;
            }
        ...
        /// <summary>
        /// 現在のネットワークカメラ選択をそのまま反映する。
        /// </summary>
        private async Task SwitchSelectedNetworkCameraAsync()
        {
            var selectedCamera = GetSelectedNetworkCamera();
            ...
            if (cameraService.SelectedNetworkCameraId == selectedCamera.CameraId
                && cameraService.SelectedSourceType == CameraSourceType.NetworkRtsp
            ...
            {
                MessageBox.Show(this, ex.Message, "ネットワークカメラ切替エラー", MessageBoxButtons.OK, MessageBoxIcon.Error);
                LoadNetworkCameras();
            }
            finally
            {
        ...
        /// ネットワークカメラ設定画面を開く。
        /// </summary>
        private void NetworkCameraMasterButton_Click(object? sender, EventArgs e)
        {
            using var dialog = new frmNetworkCameraMaster();
        ...
        private void UpdateUiState()
        {
            ...
            var selectedCameraLabel = cameraService.SelectedDeviceName ?? (isReady ? "カメラ" : "未検出");
            if (!string.IsNullOrWhiteSpace(cameraService.SelectedDeviceName))
            {
                var sourceLabel = cameraService.SelectedSourceType == CameraSourceType.NetworkRtsp ? "RTSP" : "ローカル";
                selectedCameraLabel = $"{cameraService.SelectedDeviceName} ({sourceLabel})";
            }
            ...
            buttonApplyNetworkCamera.Enabled = !isRecording && !cameraSwitching && networkCameraComboBox.SelectedItem is NetworkCameraOption;
        ...
        /// <summary>
        /// ネットワークカメラ候補をコンボボックスへ設定する。
        /// </summary>
        private void LoadNetworkCameras()
        {
            var currentCameraId = cameraService.SelectedNetworkCameraId;
            var networkCameras = networkCameraCatalog.GetNetworkCameras();

            networkCameraSelectionSyncing = true;
            networkCameraComboBox.BeginUpdate();
            networkCameraComboBox.Items.Clear();
            foreach (var networkCamera in networkCameras)
            {
                networkCameraComboBox.Items.Add(networkCamera);
            }

            if (networkCameraComboBox.Items.Count > 0)
            {
                networkCameraComboBox.SelectedIndex = FindNetworkCameraIndex(networkCameras, currentCameraId);
            }

            networkCameraComboBox.EndUpdate();
            networkCameraSelectionSyncing = false;
        }
        ...
        /// <summary>
        /// 選択中のネットワークカメラ候補を取得する。
        /// </summary>
        /// <returns>ネットワークカメラ候補。</returns>
        private NetworkCameraOption? GetSelectedNetworkCamera()
        {
            return networkCameraComboBox.SelectedItem as NetworkCameraOption;
        }
        ...
        /// <summary>
        /// 識別子に一致するネットワークカメラ候補位置を探す。
        /// </summary>
        /// <param name="networkCameras">ネットワークカメラ候補一覧。</param>
        /// <param name="cameraId">現在のカメラ識別子。</param>
        /// <returns>候補位置。</returns>
        private static int FindNetworkCameraIndex(IReadOnlyList<NetworkCameraOption> networkCameras, string? cameraId)
        {
            if (!string.IsNullOrWhiteSpace(cameraId))
            {
                for (var index = 0; index < networkCameras.Count; index++)
                {
                    if (networkCameras[index].CameraId.Equals(cameraId, StringComparison.OrdinalIgnoreCase))
                    {
                        return index;
                    }
                }
        ...
        /// <summary>
        /// サービス側の現在選択へネットワークカメラのコンボ表示を合わせる。
        /// </summary>
        private void SyncSelectedNetworkCamera()
        {
            if (networkCameraSelectionSyncing || networkCameraComboBox.Items.Count == 0)
            {
                return;
            }

            var selectedNetworkCameraId = cameraService.SelectedNetworkCameraId;
            if (string.IsNullOrWhiteSpace(selectedNetworkCameraId))
            {
                return;
            }

            for (var index = 0; index < networkCameraComboBox.Items.Count; index++)
            {
                if (networkCameraComboBox.Items[index] is not NetworkCameraOption option)
                {
                    continue;
                }

                if (!option.CameraId.Equals(selectedNetworkCameraId, StringComparison.OrdinalIgnoreCase))
                {
                    continue;
                }

                if (networkCameraComboBox.SelectedIndex == index)
                {
                    return;
                }

                networkCameraSelectionSyncing = true;
                networkCameraComboBox.SelectedIndex = index;
                networkCameraSelectionSyncing = false;
                return;
            }
        }
        ...
        private void InitializeTheme()
        {
            ...
            StyleCommandButton(buttonApplyNetworkCamera, Color.FromArgb(20, 35, 60), Color.FromArgb(117, 197, 255));
            ...
            StyleCommandButton(buttonNetworkCameraMaster, Color.FromArgb(25, 55, 88), Color.FromArgb(128, 210, 255));
            ...
            networkCameraComboBox.BackColor = Color.FromArgb(16, 24, 38);
            networkCameraComboBox.ForeColor = Color.White;
        ...
        private void ApplyAppIcons()
        {
            ...
            buttonNetworkCameraMaster.Image = UiIconFactory.CreateButtonBitmap(ButtonIconKind.Settings);
            ...
            buttonNetworkCameraMaster.ImageAlign = ContentAlignment.MiddleLeft;
            buttonExit.ImageAlign = ContentAlignment.MiddleLeft;
            buttonNetworkCameraMaster.TextImageRelation = TextImageRelation.ImageBeforeText;
        }
